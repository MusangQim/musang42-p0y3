...
def main() -> None:
    ...
    only_capital = [player for player in names if player == player.capitalize()]
    # or it can use:
    ...
    score_dict = {name: random.randint(1, 1000) for name in all_capital}
    print(f"Score dict: {score_dict}")
    score_average = sum(score_dict.values()) / len(score_dict)
    print(f"Score average is {score_average:.2f}")
    score_high = {name: score for name, score in score_dict.items() if score > score_average}
